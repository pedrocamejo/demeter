...
import cpc.demeter.controlador.ministerio.ContTipoRiego;
...
public class ContTiposRiego extends ContCatalogo<TipoRiego> implements
		EventListener, Serializable {

	private static final long serialVersionUID = -8012014410778713889L;
	private AppDemeter app;

	public ContTiposRiego(AccionFuncionalidad accionesValidas, AppDemeter app)
		...
		NegocioTipoRiego servicios = NegocioTipoRiego.getInstance();
		this.app = app;
		dibujar(accionesValidas, "Tipos de Riego", servicios.getTodos(), app);
	...
	public void onEvent(Event event) throws Exception {
		try {
			...
			if (accion <= Accion.CONSULTAR) {
				NegocioTipoRiego servicio = NegocioTipoRiego.getInstance();
				TipoRiego tipo = getDatoSeleccionado();
				servicio.setModelo(tipo);
				if (tipo == null && accion != Accion.AGREGAR)
					app.mostrarError("Seleccione un tipo de riego");
				else
					new ContTipoRiego(accion, servicio.getModelo(), this, app);
			}
	...
	@SuppressWarnings("unchecked")
	public List cargarDato(TipoRiego tipo) {
