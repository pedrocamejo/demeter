...
import cpc.demeter.controlador.gestion.ContEntradaActivo;
...
public class ContEntradasActivos extends ContCatalogo<TrasladoActivo> implements
	...
	public ContEntradasActivos(AccionFuncionalidad accionesValidas,
		...
		this.app = app;
		dibujar(accionesValidas, "ENTRADAS DE ACTIVOS",
				negocio.obtenerEntradas(), app);
		...
		try {
			...
			if (accion <= Accion.CONSULTAR || accion == Accion.ANULAR) {
				...
				if (entrada == null && accion != Accion.AGREGAR)
					...
					new ContEntradaActivo(accion, entrada, this, app);
			}
