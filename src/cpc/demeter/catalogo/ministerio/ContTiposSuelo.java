...
import cpc.demeter.controlador.ministerio.ContTipoSuelo;
...
public class ContTiposSuelo extends ContCatalogo<TipoSuelo> implements
		EventListener, Serializable {

	private static final long serialVersionUID = -8012014410778713889L;
	private AppDemeter app;

	public ContTiposSuelo(AccionFuncionalidad accionesValidas, AppDemeter app)
		...
		NegocioTipoSuelo servicios = NegocioTipoSuelo.getInstance();
		this.app = app;
		dibujar(accionesValidas, "Tipos de Suelo", servicios.getTodos(), app);
	...
	public void onEvent(Event event) throws Exception {
		try {
			...
			if (accion <= Accion.CONSULTAR) {
				NegocioTipoSuelo servicio = NegocioTipoSuelo.getInstance();
				TipoSuelo tipo = getDatoSeleccionado();
				servicio.setModelo(tipo);
				if (tipo == null && accion != Accion.AGREGAR)
					app.mostrarError("Seleccione un tipo de suelo");
				else
					new ContTipoSuelo(accion, servicio.getModelo(), this, app);
			}
	...
	@SuppressWarnings("unchecked")
	public List cargarDato(TipoSuelo tipo) {
