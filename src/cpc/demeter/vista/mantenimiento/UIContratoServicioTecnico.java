...
public class UIContratoServicioTecnico extends
		CompVentanaMaestroDetalle<ContratoServicioTecnico, DetalleContrato> {
	...
	private CompLista<DetalleContratoSalidaArticulo> compListaSalidas;
	private CompLista<DetalleContratoDevolucionArticulo> compListaDevoluciones;

	public UIContratoServicioTecnico(String titulo, int ancho,
			List<Cliente> clientes, List<SolicitudServicioTecnico> solicitudes,
	...
	public UIContratoServicioTecnico(ContratoServicioTecnico modeloIn,
			String titulo, int ancho) throws ExcDatosInvalidos {
		super(titulo, ancho);
		setModelo(modeloIn);
	}
	...
	public void inicializar() {
		...
		compListaDevoluciones = new CompLista<DetalleContratoDevolucionArticulo>(
				EncabezadoDevoluciones());
		compListaSalidas = new CompLista<DetalleContratoSalidaArticulo>(
				EncabezadoSalidas());

		getGbDetalle().appendChild(compListaDevoluciones);

		getGbDetalle().appendChild(compListaSalidas);
	...
	public void cargarValores(ContratoServicioTecnico arg0)
		...
		ContContratoServicioTecnico controlador = (ContContratoServicioTecnico) getControlador();
		if (getModelo().getTipoContrato() != null) {
		...
		if (getModelo().getSolicitud() != null) {
			...
			getBinder().addBinding(solicitud, "seleccion",
					...
					null, null, null);
			solicitud.setText(getModelo().getStrSolicitud());
		}

		/*
		 * getBinder().addBinding(unidadProduccion, "seleccion",
		 * getNombreModelo()+".unidadProductiva", null, null, "save", null,
		 * null, null, null); if (getModelo().getUnidadProductiva() != null){
		 * try {
		 * controlador.actualizarProductor(getModelo().getUnidadProductiva(
		 * ).getProductor());
		 * unidadProduccion.setSeleccion(getModelo().getUnidadProductiva()); }
		 * catch (ExcFiltroExcepcion e) { e.printStackTrace(); } }
		 */
		...
		if (getModelo().getPagador() != null) {
			cedulaProductor.setSeleccion(getModelo().getPagador());
			getLblNombreSolicitante().setValue(
					getModelo().getPagador().getNombres());
			lblDireccionPagador.setValue(getModelo().getPagador()
					.getDireccion());
		}
		...
		if (getModelo().getDetalleContratoSalidaArticulos() != null) {
			compListaSalidas.setModelo(getModelo()
					.getDetalleContratoSalidaArticulos());
			System.out.println("a1");
		}

		if (getModelo().getDetalleContratoDevolucionArticulos() != null) {
			compListaDevoluciones.setModelo(getModelo()
					.getDetalleContratoDevolucionArticulos());
	 ...
	 * titulo.setMetodoBinder("getDescripcion"); titulo.setOrdenable(true);
	 * encabezado.add(titulo);
	 * 
	 * 
	 * titulo = new CompEncabezado("Serv/Art",100);
	 * titulo.setMetodoBinder("getStrTipoProducto"); encabezado.add(titulo);
	 * 
	 * titulo = new CompEncabezado("Tipo",150);
	 * titulo.setMetodoBinder("getStrTipo"); encabezado.add(titulo);
	 * 
	 * titulo = new CompEncabezado("Clase",150);
	 * titulo.setMetodoBinder("getStrClase"); encabezado.add(titulo);
	 * 
	 * titulo = new CompEncabezado("Producto",250);
	 * titulo.setMetodoBinder("getDescripcion"); titulo.setOrdenable(true);
	 * encabezado.add(titulo);
	...
	private List<CompEncabezado> encabezadosDetalle() {
		 ...
		 * titulo.setMetodoComponente("seleccion");
		 * titulo.setMetodoBinder("getProducto");
		 * titulo.setMetodoModelo(".producto"); //titulo.setOrdenable(true);
		 * encabezado.add(titulo);
		 */
	...
	private ArrayList<Component> cargarLista() {
		...
		System.out.println("articulos " + productos.size());
		try {
	...
	public List EncabezadoDevoluciones() {
		CompEncabezado titulo;
		List<CompEncabezado> encabezado = new ArrayList<CompEncabezado>();

		titulo = new CompEncabezado("Nro Control", 250);
		titulo.setOrdenable(true);
		titulo.setMetodoBinder("getDevolucionArticulo");
		encabezado.add(titulo);
	...
	@SuppressWarnings("unchecked")
	public List EncabezadoSalidas() {
		CompEncabezado titulo;
		List<CompEncabezado> encabezado = new ArrayList<CompEncabezado>();

		titulo = new CompEncabezado("Nro Control", 250);
		titulo.setOrdenable(true);
		titulo.setMetodoBinder("getSalidaExternaArticulo");
		encabezado.add(titulo);
	...
	public CompLista<DetalleContratoSalidaArticulo> getCompListaSalidas() {
		return compListaSalidas;
	}

	public void setCompListaSalidas(
			CompLista<DetalleContratoSalidaArticulo> compListaSalidas) {
		this.compListaSalidas = compListaSalidas;
	}

	public CompLista<DetalleContratoDevolucionArticulo> getCompListaDevoluciones() {
		return compListaDevoluciones;
	}

	public void setCompListaDevoluciones(
			CompLista<DetalleContratoDevolucionArticulo> compListaDevoluciones) {
		this.compListaDevoluciones = compListaDevoluciones;
	}
