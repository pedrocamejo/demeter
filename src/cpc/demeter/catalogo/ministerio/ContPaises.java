...
import cpc.demeter.controlador.ministerio.ContPais;
import cpc.modelo.ministerio.dimension.UbicacionPais;
import cpc.negocio.ministerio.basico.NegocioPais;
...
public class ContPaises extends ContCatalogo<UbicacionPais> implements
		EventListener, Serializable {

	private static final long serialVersionUID = -8012014410778713889L;
	private AppDemeter app;

	public ContPaises(AccionFuncionalidad accionesValidas, AppDemeter app)
		...
		NegocioPais servicios = NegocioPais.getInstance();
		this.app = app;
		dibujar(accionesValidas, "Paises", servicios.getTodos(), app);
	...
	public void onEvent(Event event) throws Exception {
		try {
			...
			if (accion <= Accion.CONSULTAR) {
				NegocioPais servicio = NegocioPais.getInstance();
				UbicacionPais tipo = getDatoSeleccionado();
				servicio.setModelo(tipo);
				if (tipo == null && accion != Accion.AGREGAR)
					app.mostrarError("Seleccione un tipo de riego");
				else
					new ContPais(accion, servicio.getModelo(), this, app);
			}
	...
	@SuppressWarnings("unchecked")
	public List cargarDato(UbicacionPais tipo) {
