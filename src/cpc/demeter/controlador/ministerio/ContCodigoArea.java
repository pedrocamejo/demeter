...
import cpc.demeter.catalogo.ministerio.ContCodigosArea;
import cpc.demeter.vista.ministerio.UiCodigoArea;
...
public class ContCodigoArea extends ContVentanaBase<CodigoArea> {

	private static final long serialVersionUID = 6184414588153046382L;
	private NegocioCodigoArea servicio;
	private AppDemeter app;
	private UiCodigoArea vista;

	public ContCodigoArea(int modoOperacion, CodigoArea tipo,
			ContCodigosArea llamador, AppDemeter app) throws ExcDatosInvalidos,
		...
		try {
			if (tipo == null || modoAgregar())
				tipo = new CodigoArea();
			setear(tipo, new UiCodigoArea("Codigo Area Telefonico ("
					+ Accion.TEXTO[modoOperacion] + ")", 550), llamador, app);
			vista = ((UiCodigoArea) getVista());
	...
	public void eliminar() {
		try {
			servicio = NegocioCodigoArea.getInstance();
