...
import cpc.demeter.vista.UICostoBancario;
...
public class ContCostoBancario extends ContVentanaBase<CostoBancario> {

	private static final long serialVersionUID = 6184414588153046382L;
	private AppDemeter app;
	private UICostoBancario vista;
	private NegocioCostoBancario negocio;

	public ContCostoBancario(int modoOperacion, CostoBancario labor,
			ICompCatalogo<CostoBancario> llamador, AppDemeter app)
			throws ExcDatosInvalidos, ExcSeleccionNoValida {
		super(modoOperacion);
		this.app = app;
		try {
			negocio = NegocioCostoBancario.getInstance();
			...
			vista = new UICostoBancario("Costo Bancario ("
					+ Accion.TEXTO[modoOperacion] + ")", 590, impuestos);
			setear(labor, vista, llamador, app);
			vista = ((UICostoBancario) getVista());
	...
	public void onEvent(Event event) throws Exception {
		 ...
		 * == vista.getCmbTipo()){ actualizarClases(); }
	...
	/*
	 * public void actualizarClases() throws ExcFiltroExcepcion{ try{ if
	 * (vista.getCmbTipo().getSeleccion() != null){ negocio =
	 * NegocioCostosBancarios.getInstance();
	 * //vista.refrescarClase(negocio.getClases
	 * (vista.getTipo().getSeleccion())); } }catch (NullPointerException e) { //
