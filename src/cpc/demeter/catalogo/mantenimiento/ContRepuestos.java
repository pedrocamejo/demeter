...
public class ContRepuestos extends ContCatalogo<Repuesto> implements
		EventListener, Serializable {

	private static final long serialVersionUID = -8012014410778713889L;
	private AppDemeter app;

	public ContRepuestos(AccionFuncionalidad accionesValidas, AppDemeter app)
		...
		NegocioRepuesto servicios = NegocioRepuesto.getInstance();
		this.app = app;
		dibujar(accionesValidas, "repuestoS", servicios.getTodos(), app);
	...
	public void onEvent(Event event) throws Exception {
		try {
			...
			if (accion <= Accion.CONSULTAR) {
				NegocioRepuesto servicio = NegocioRepuesto.getInstance();
				Repuesto repuesto = getDatoSeleccionado();
				servicio.setRepuesto(repuesto);
				if (repuesto == null && accion != Accion.AGREGAR)
					app.mostrarError("Seleccione un Banco");
				else
					new ContRepuesto(accion, servicio.getRepuesto(), this, app);
			}
	...
	@SuppressWarnings("unchecked")
	public List cargarDato(Repuesto dato) {
