...
import cpc.demeter.controlador.ministerio.ContTipoUsoTierra;
...
public class ContTiposUsosTierra extends ContCatalogo<TipoUsoTierra> implements
		EventListener, Serializable {

	private static final long serialVersionUID = -8012014410778713889L;
	private AppDemeter app;

	public ContTiposUsosTierra(AccionFuncionalidad accionesValidas,
			...
			ExcFiltroExcepcion {
		NegocioTipoUsoTierra servicios = NegocioTipoUsoTierra.getInstance();
		this.app = app;
		dibujar(accionesValidas, "Tipos de uso de la Tierra",
	...
	public void onEvent(Event event) throws Exception {
		try {
			...
			if (accion <= Accion.CONSULTAR) {
				NegocioTipoUsoTierra servicio = NegocioTipoUsoTierra
						.getInstance();
				TipoUsoTierra tipo = getDatoSeleccionado();
				servicio.setModelo(tipo);
				if (tipo == null && accion != Accion.AGREGAR)
					app.mostrarError("Seleccione un tipo de suelo");
				else
					new ContTipoUsoTierra(accion, servicio.getModelo(), this,
							app);
			}
	...
	@SuppressWarnings("unchecked")
	public List cargarDato(TipoUsoTierra tipo) {
