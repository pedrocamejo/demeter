...
import cpc.demeter.vista.mantenimiento.UITipoFalla;
...
public class ContTipoFalla extends ContVentanaBase<TipoFalla> {

	private static final long serialVersionUID = 1180434079481225766L;
	private NegocioTipoFalla servicio;
	private UITipoFalla vista;
	private AppDemeter app;

	public ContTipoFalla(int modoOperacion, TipoFalla momento,
			ContCatalogo<TipoFalla> llamador, AppDemeter app)
			throws ExcDatosInvalidos {
		super(modoOperacion);
		this.app = app;
		this.servicio = NegocioTipoFalla.getInstance();
		try {
			if (datoNulo(momento) || modoAgregar()) {
				momento = new TipoFalla();
			}
		} catch (ExcSeleccionNoValida e) {

			e.printStackTrace();
		}
		setear(momento, new UITipoFalla("Tipo Falla  ("
				+ Accion.TEXTO[modoOperacion] + ")", 550, this.app), llamador,
				this.app);
		this.vista = (UITipoFalla) getVista();
