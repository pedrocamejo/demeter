...
import cpc.demeter.controlador.mantenimiento.ContSistema;
...
public class ContSistemas extends ContCatalogo<Sistema> {
	...
	public ContSistemas(AccionFuncionalidad accionesValidas, AppDemeter app)
			throws ExcColumnasInvalidas, ExcArgumentoInvalido,
			ExcFiltroExcepcion {
		NegocioSistemaBienProduccion sistemas = NegocioSistemaBienProduccion
				.getInstance();
		this.app = app;
		dibujar(accionesValidas,
				"Sistemas que pertenecen al Bien de Produccion",
				sistemas.getTodos(), app);
	}

	public ContSistemas(int modoOperacion) {
		super(modoOperacion);
	}
	...
	@SuppressWarnings("unchecked")
	@Override
	public List cargarDato(Sistema arg0) {
	...
	public void onEvent(Event event) throws Exception {
		try {
			...
			if (accion <= Accion.CONSULTAR || accion == Accion.ANULAR) {
				Sistema sistema = getDatoSeleccionado();
				new ContSistema(accion, sistema, this, app);
			} else {
