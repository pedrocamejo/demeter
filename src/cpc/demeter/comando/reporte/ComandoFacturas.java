...
public class ComandoFacturas implements IComando, EventListener {
	...
	public void menuImprimir() throws ExcFiltroExcepcion {
		...
		vistaReporte.setTitle("Facturas por Dia");
	...
	public void imprimir() {
		try {
			...
			if (tipo.equals("Preservicios")) {
				JRDataSource ds = new JRBeanCollectionDataSource(
						negocio.getTodosprefecha(
								String.format("%1$td/%1$tm/%1$tY", inicio),
								String.format("%1$td/%1$tm/%1$tY", fin)));

				app.agregarReporte();
				Jasperreport reporte = app.getReporte();
				reporte.setParameters(parametros);
				reporte.setDatasource(ds);
				if(tipoReporte!=null &&tipoReporte.equals("xls")){
					parametros.put("exportParameter", app.getXLSParameters(false));
					reporte.setSrc("reportes/facturasDiaXls.jasper");
					reporte.setType("xls");
					}
					else {
			...
			else if (tipo.equals("Postservicios")) {
				JRDataSource ds = new JRBeanCollectionDataSource(
						negocio.getTodosPostfecha(
								String.format("%1$td/%1$tm/%1$tY", inicio),
								String.format("%1$td/%1$tm/%1$tY", fin)));
				app.agregarReporte();
				Jasperreport reporte = app.getReporte();
