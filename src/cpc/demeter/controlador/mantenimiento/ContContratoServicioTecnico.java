...
public class ContContratoServicioTecnico extends
		ContVentanaMaestroDetalle<ContratoServicioTecnico, DetalleContrato> {

	private static final long serialVersionUID = 3482809265694914317L;
	private NegocioContratoServicioTecnico negocioContrato;
	private UIContratoServicioTecnico vistaContrato;
	private AppDemeter app;

	public ContContratoServicioTecnico(int modoOperacion,
			ContratoServicioTecnico contrato,
			ContCatalogo<ContratoServicioTecnico> llamador, AppDemeter app)
			throws ExcDatosInvalidos, ExcSeleccionNoValida {
		super(modoOperacion);
		this.app = app;
		List<Cliente> clientes = null;
		List<SolicitudServicioTecnico> solicitudes = null;
		...
		try {
			clientes = negocioContrato.getClientes();
		...
		setear(contrato, new UIContratoServicioTecnico(
				"Contrato ServicioTecnico", 730, clientes, solicitudes,
				tiposContrato, planesFinaciamineto, articulos), llamador, app);
		vistaContrato = (UIContratoServicioTecnico) getVista();
		vistaContrato.activarDetalle();
	...
	public void guardar() {
		...
		//el binder sobreescribe el guardar se trabaja el detalle a mano
	...
	public ContratoServicioTecnico cargarModelo() throws ExcAgregacionInvalida {
		ContratoServicioTecnico ctto = getDato();
		Double impuestoa = 0.0;
		Double subtotala = 0.0;
		//el binder sobreescribe el guardar se trabaja el detalle a mano
		List<DetalleContrato> detalles = vistaContrato.getDetalle().getColeccion();
		for (DetalleContrato detalleContrato : detalles) {
			detalleContrato.setPrecioUnidad((detalleContrato.getPrecioUnidad()));
			detalleContrato.setSubtotal((detalleContrato.getSubtotal()));
		}
	    ctto.setDetallesContrato(detalles);
	//	ctto.setDetallesContrato(vistaContrato.getModeloDetalle());
		ctto.setPagador(vistaContrato.getCedulaPagador().getSeleccion());
		ctto.setSolicitud(vistaContrato.getSolicitud().getSeleccion());
		// ctto.setUnidadProductiva(vistaContrato.getUnidadProduccion().getSeleccion());
		...
		for (DetalleContrato item : ctto.getDetallesContrato()) {
			...
			subtotala+=item.getSubtotal();
			impuestoa+=(1 + item.getProducto().getImpuesto().getPorcentaje() / 100);
			//total += (Real.redondeoMoneda(item.getSubtotal())*Real.redondeoMoneda((1 + item.getProducto().getImpuesto().getPorcentaje() / 100)));
		}
		if (ctto.getTipoContrato().getId() == 2) {
			ctto.setTotal(0.0);
			ctto.setSaldo(0.0);
		} else {
			total=Real.redondeoMoneda(subtotala)+Real.redondeoMoneda(impuestoa);
			ctto.setTotal(total);
			ctto.setSaldo(total);
		}

		List<SalidaExternaArticulo> salidas = negocioContrato
				.getSalidaExternaArticulos(vistaContrato.getSolicitud()
						.getSeleccion());

		List<DetalleContratoSalidaArticulo> detalleSalidas = new ArrayList<DetalleContratoSalidaArticulo>();

		for (SalidaExternaArticulo SalidaArticulo : salidas) {
			DetalleContratoSalidaArticulo detalle = new DetalleContratoSalidaArticulo();
			detalle.setContratoServicioTecnico(ctto);
			detalle.setSalidaExternaArticulo(SalidaArticulo);
			detalleSalidas.add(detalle);
		}

		ctto.setDetalleContratoSalidaArticulos(detalleSalidas);

		List<DevolucionArticulo> devoluciones = negocioContrato
				.getDevolucionArticulos(vistaContrato.getSolicitud()
						.getSeleccion());

		List<DetalleContratoDevolucionArticulo> detalleDevolucion = new ArrayList<DetalleContratoDevolucionArticulo>();

		for (DevolucionArticulo detalleContratoDevolucionArticulo : devoluciones) {
			DetalleContratoDevolucionArticulo devolucion = new DetalleContratoDevolucionArticulo();
			devolucion.setContratoServicioTecnico(ctto);
			devolucion.setDevolucionArticulo(detalleContratoDevolucionArticulo);
			detalleDevolucion.add(devolucion);
		}
		ctto.setDetalleContratoDevolucionArticulos(detalleDevolucion);
	...
	public void validar() throws WrongValuesException, ExcEntradaInconsistente {
		 ...
		 */
		/*
		 * if (vistaContrato.getUnidadProduccion().getSeleccion()==null) throw
		 * new WrongValueException(vistaContrato.getUnidadProduccion(),
	...
	public void onEvent(Event event) throws Exception {
		...
		if (event.getTarget() == getVista().getAceptar()) {
			...
			if (event.getTarget() == vistaContrato.getCedulaPagador()) {
				...
				SolicitudServicioTecnico solicitud = vistaContrato
						.getSolicitud().getSeleccion();
				vistaContrato.getCedulaProductor().setSeleccion(
						solicitud.getProductor());
				actualizarProductor(vistaContrato.getCedulaProductor()
						.getSeleccion());
				NegocioContratoServicioTecnico aux = negocioContrato
						.getInstance();
				vistaContrato.getTipoContrato().setSeleccion(
						aux.getServicioGravado());
				vistaContrato.getPlanFinanciamiento().setSelectedIndex(0);
				vistaContrato.getPlanFinanciamiento().setDisabled(true);
	...
	public void actualizarDatosSolicitud(SolicitudServicioTecnico solicitud)
	...
	public void actualizarProductor(Cliente cliente) throws ExcFiltroExcepcion {
		if (cliente != null) {
			...
			vistaContrato.getLblNombreSolicitante().setValue(
					cliente.getNombres());
			// vistaContrato.getUnidadProduccion().setModelo(negocioContrato.getDirecciones(cliente));
		}
	}
	...
	public void actualizarServicio(CompBuscar<IProducto> componente)
		...
		try {// ojo
			Double precio;
			if (producto.getPrecioBase(tipo) == null)
				precio = (((ArticuloVenta) producto)
						.getPrecio());
			else
				precio = (producto.getPrecioBase(tipo));
	...
	//	acuDesc = 0;
		...
		vistaContrato.getTxtTotal().setValue(Real.redondeoMoneda(bruto));
		vistaContrato.getLblMontoContrato().setValue("" + Real.redondeoMoneda(bruto));
		actualizarFinanciamiento();
		/*
