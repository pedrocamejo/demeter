...
public class ContCotizacionesServicio extends ContCatalogo<Cotizacion> implements
		EventListener {
	...
	public ContCotizacionesServicio(AccionFuncionalidad accionesValidas, AppDemeter app)		throws SQLException, ExcAccesoInvalido, PropertyVetoException,
		...
		dibujar(accionesValidas, "COTIZACIONES MANTENIMIENTO ",	negocioCotizacion.getCotizacionesExpedeinteMantenimiento(), app);
		this.app = app;
	}
	...
	try {
			...
			if (accion <= Accion.CONSULTAR || accion == Accion.ANULAR || accion == Accion.PROCESAR)
			{
				...
				if (cotizacion == null || accion == Accion.AGREGAR)
				{
					...
					TipoCotizacion tipo = negocioCotizacion.getMantenimiento();
