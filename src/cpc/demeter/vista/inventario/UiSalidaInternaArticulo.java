...
public class UiSalidaInternaArticulo
		extends
		CompVentanaMaestroDetalle<SalidaInternaArticulo, DetalleSalidaInternaArticulo> {
	...
	private CompBuscar<Trabajador> cmbTrabajadores;
	private CompBuscar<Activo> cmbActivo;
	private CompGrupoBusqueda<Consumible> auxconsumible;

	private Label lblDatosActivo;

	private CompGrupoBusqueda<Consumible> auxConsumible;
	private CompGrupoBusqueda<Herramienta> auxHerramienta;
	private CompGrupoBusqueda<Repuesto> auxRepuesto;

	public UiSalidaInternaArticulo(String titulo, int ancho, int modo,
		...
		setModelo(salidaInternaArticulo);
		this.accion = modo;
	...
	public void inicializar() {
		...
		cmbActivo = new CompBuscar<Activo>(getEncabezadoActivo(), 5);
		cmbActivo.setModelo(activos);
		cmbActivo.setAncho(650);
		cmbTrabajadores = new CompBuscar<Trabajador>(getEncabezadoTrabajador(),
				2);
		cmbTrabajadores.setModelo(trabajadores);
		cmbTrabajadores.setAncho(650);
		// servicio =NegocioConsumible.getInstance();
		// setGbDetalle("Consumibles Equivalentes");

		lblDatosActivo = new Label();
		...
		cmbActivo.setListenerEncontrar(getControlador());
	...
	public void dibujar() {
		...
		cgdDetalles.addComponente("TrabajadorSolicitante", cmbTrabajadores);
		cgdDetalles.addComponente("Activo", cmbActivo);
		cgdDetalles.addComponente("DatosActivo", lblDatosActivo);

		// Cmbgrupoconsumible.addComponente(" ",null);

		// setDetalles(cargarLista(), getModelo().getConsumibleEquivalente(),
		// encabezadosPrimario());
		List<DetalleSalidaInternaArticulo> a = getModelo()
				.getDetalleSalidaInternaArticulos();
		setDetalles(cargarLista(), a, encabezadosPrimariodos());
		getDetalle().setNuevo(new DetalleSalidaInternaArticulo());
	...
	public void cargarValores(SalidaInternaArticulo salidaInternaArticulo)
			throws ExcDatosInvalidos {

		if (accion != Accion.AGREGAR) {
			txtUsuario.setValue(salidaInternaArticulo.getUsuario());
			// getBinder().addBinding (txtCodCCNU, "value",
			// getNombreModelo()+".descripcion", null, null, "save", null, null,
			// null, null);
			txtFecha.setValue(salidaInternaArticulo.getFecha().toString());

			txtNumeroControl.setValue(salidaInternaArticulo
					.getStrNroDocumento());

			cmbActivo.setSeleccion(salidaInternaArticulo.getActivo());
			lblDatosActivo.setMultiline(true);
			lblDatosActivo.setValue(salidaInternaArticulo.getActivo()
					.getDescripcionLarga());

			cmbTrabajadores.setSeleccion(salidaInternaArticulo.getTrabajador());
			cmbTrabajadores.setText(salidaInternaArticulo.getTrabajador().getNombreCompleto());
	...
	public CompBuscar<Trabajador> getCmbTrabajadores() {
		return cmbTrabajadores;
	}

	public void setCmbTrabajadores(CompBuscar<Trabajador> cmbTrabajadores) {
		this.cmbTrabajadores = cmbTrabajadores;
	}

	public CompBuscar<Activo> getCmbActivo() {
		return cmbActivo;
	}

	public void setCmbActivo(CompBuscar<Activo> cmbActivo) {
		this.cmbActivo = cmbActivo;
	}
	...
	private List<CompEncabezado> getEncabezadoActivo() {
		...
		titulo = new CompEncabezado("Codigo. ", 90);
		titulo.setMetodoBinder("getCodigoActivo");
		lista.add(titulo);

		titulo = new CompEncabezado("Ejemplar. ", 90);
		titulo.setMetodoBinder("getIdEjemplarActivo");
		lista.add(titulo);

		titulo = new CompEncabezado("Serial. ", 90);
		titulo.setMetodoBinder("getSerial");
		lista.add(titulo);

		titulo = new CompEncabezado("Denominación. ", 150);
		titulo.setMetodoBinder("getNombre");
		lista.add(titulo);

		titulo = new CompEncabezado("Marca. ", 90);
		titulo.setMetodoBinder("getDescripcionMarca");
		lista.add(titulo);

		titulo = new CompEncabezado("Modelo. ", 90);
	...
	public void activarConsulta() {
		...
		getCmbTrabajadores().setDisabled(true);
		getCmbActivo().setDisabled(true);
		desactivarDetalle();
	}
	...
	public Label getLblDatosActivo() {
		return lblDatosActivo;
	}

	public void setLblDatosActivo(Label lblDatosActivo) {
		this.lblDatosActivo = lblDatosActivo;
	}
